          ...
          @if(isset($categories_new))
            ...
            <ul class="new-list">
              ...
              @foreach($categories_new as $cat => $cate)
                <li><a href="/news/category/{{ $cate->slug . '.html' }}"><?php echo $cate->category_name; ?></a></li>
              @endforeach
            </ul>
            @endif
          @endif
          ...
          @if(isset($new_same_cate))
            ...
            <ul class="new-list">
              @foreach($new_same_cate as $news => $new_cate)
                <div class="list-bv">
                  <div class="cover-img"><img src="/image/{{$new_cate->image}}" alt="">
                  </div>
            ...
            @if (isset($tags))
              @if(!empty($tags))
                @foreach ($tags as $tags => $tag)
                  <a href="/news/tags/{{ $tag->tag_name }}" class="btn">{{ $tag->tag_name }}</a>
                @endforeach
              @endif
            @endif
          </div>
...
<script type="text/javascript">
  function openTabFb() {
    window.open('https://www.facebook.com/sharer/sharer.php?u='+window.location.href, 'facebook', 'height = 500, width = 500' );
  }
  function openTabGg() {
    window.open('https://plus.google.com/share?url='+window.location.href, 'google', 'height = 500, width = 500' );
  }
  function openTabTw() {
    window.open('https://twitter.com/intent/tweet?url='+window.location.href, 'tweet', 'height = 500, width = 500' );
  }
</script>
@endsection
