...
<div class="mpsw-about-title mpsw-jp-service bg-case-studies">
  <div class="container">
    <div class="content-page-title">
      <p class="sub-title"><a href="#">Home</a> / <a>News</a></p>
...
<div class="mpsw-content">
  <div class="mpsw-new m-b-100">
    <div class="container">
      <h1 class="mpsw-heading">{{$category->name}}</h1>
          ...
          @if (isset($news))
            @foreach ($news as $key => $new)
              <div class="content-new">
                <h3 class="txt-first">{{$new->news_title}}</h3>
                <span>{{(!empty($new->created_at)) ? date('Y/m/d', strtotime($new->created_at)) : ''}}</span>
                <div class="cover-img"><img src="http://admin.m/image/{{$new->image}}" alt="image"></div>
                {!!$new->description!!}
                <a title="{{$new->news_title}}" href="/news/{{$category->slug.'/'.$new->slug}}" style="color: white;"><button class="btn btn-read-more">
                READ MORE</button></a>
              </div>
              <hr>
            @endforeach
              ...
              @foreach($news_popular as $new_s => $newss)
                <div class="list-bv">
                  ...
                  <div class="content-img">
                    ...
                    <span>{!! ($newss->created_at != '') ? date('Y/m/d', strtotime($newss->created_at)) : '' !!}</span>
                  </div>
                </div>
              @endforeach
            </ul>
